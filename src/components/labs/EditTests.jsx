...
const EditScreen = () => {
  ...
  const fetchTestDetails = async () => {
    try {
      const response = await fetch(
        "https://server.bookmyappointments.in/api/bma/hospital/singletest",
        {
          method: "POST",
          headers: {
      ...
      const updateTest = await response.json();
      setTest(updateTest.test);
      setName(updateTest.test.name);
      setConsultancyFee(updateTest.test.price.consultancyfee);
      setImageUrl(updateTest.test.image);
      setLoading(false);
    } catch (error) {
      console.error(error);
  ...
  const handleMenuItemClick = (option) => {
    ...
    if (option === "Manage Slots") {
      ...
      navigate(`/test-bookings/${test._id}`);
    }
  };
  ...
  const handleSaveTestDetails = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update test details");
      }
      setSnackbarMessage("test details updated successfully!");
      setSnackbarOpen(true);
      setLoading(false);
    } catch (error) {
      console.error(error);
      setSnackbarMessage("Failed to update test details");
      setSnackbarOpen(true);
      setLoading(false);
    }
  };
  ...
  const handleDeleteTest = async () => {
    try {
      const response = await fetch(
        `https://server.bookmyappointments.in/api/bma/hospital/deletetest/${test.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            <Typography variant="h4">Edit test</Typography>
          </Grid>
          <Grid item>
            <IconButton
              aria-label="menu"
        ...
        <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)}>
          ...
          <DialogContent>
            <DialogContentText>Are you sure you want to delete this test?</DialogContentText>
          </DialogContent>
          <DialogActions>
